...
public class LoginServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("/auth/logout".equals(path)) {
            ...
            resp.sendRedirect(ctx(req)); // Redirige a la raíz del contexto
            return;
        }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        // Solo permitir login si es asistente
        if (!"ASISTENTE".equals(rol)) {
            req.setAttribute("error", "Solo los usuarios asistentes pueden iniciar sesión en ClienteMobile.");
            req.getRequestDispatcher(JSP_LOGIN).forward(req, resp);
            return;
        }
